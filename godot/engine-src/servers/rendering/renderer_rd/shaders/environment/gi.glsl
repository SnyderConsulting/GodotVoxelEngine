...
#define SDFGI_LATTICE_CUBIC 0u
#define SDFGI_LATTICE_TRUNCATED_OCTAHEDRON 1u
...
bool sdfgi_is_valid_cell(ivec3 cell) {
	int px = cell.x & 1;
	int py = cell.y & 1;
	int pz = cell.z & 1;
	return (px == py) && (py == pz);
...
ivec3 sdfgi_nearest_cell(vec3 pos) {
	...
	for (int dz = 0; dz <= 1; dz++) {
		for (int dy = 0; dy <= 1; dy++) {
			for (int dx = 0; dx <= 1; dx++) {
				...
				if (!sdfgi_is_valid_cell(cand)) {
					continue;
				}
				vec3 delta = pos - vec3(cand);
...
void sdfvoxel_gi_process(uint cascade, vec3 cascade_pos, vec3 cam_pos, vec3 cam_normal, vec3 cam_specular_normal, float roughness, out vec3 diffuse_light, out vec3 specular_light) {
	...
	bool use_truncated = sdfgi.lattice_type == SDFGI_LATTICE_TRUNCATED_OCTAHEDRON;
	if (use_truncated) {
		ivec3 probe_posi = sdfgi_nearest_cell(cascade_pos);
		vec3 probe_pos = vec3(probe_posi);
		vec3 probe_to_pos = cascade_pos - probe_pos;
		float probe_len_sq = dot(probe_to_pos, probe_to_pos);
		vec3 probe_dir = probe_len_sq > 1e-6 ? normalize(-probe_to_pos) : cam_normal;
		float weight = max(0.005, dot(cam_normal, probe_dir));
		...
		ivec3 probe_base = probe_posi;
		ivec3 local_tex_pos = ivec3(probe_base.xy, int(cascade));
		local_tex_pos.x += probe_base.z * sdfgi.probe_axis_size;
		local_tex_pos.xy = local_tex_pos.xy * (SDFGI_OCT_SIZE + 2) + ivec2(1);

		vec3 local_diffuse_posf = (vec3(local_tex_pos) + vec3(octahedron_encode(cam_normal) * float(SDFGI_OCT_SIZE), 0.0)) * sdfgi.lightprobe_tex_pixel_size;
		vec3 local_specular_posf = (vec3(local_tex_pos) + vec3(octahedron_encode(cam_specular_normal) * float(SDFGI_OCT_SIZE), 0.0)) * sdfgi.lightprobe_tex_pixel_size;

		vec3 diffuse = textureLod(sampler2DArray(lightprobe_texture, linear_sampler), local_diffuse_posf, 0.0).rgb;
		diffuse_accum += vec4(diffuse * weight * sdfgi.cascades[cascade].exposure_normalization, weight);

		vec3 specular = vec3(0.0);
		if (roughness < 0.99) {
			specular = textureLod(sampler2DArray(lightprobe_texture, linear_sampler), local_specular_posf + vec3(0, 0, float(sdfgi.max_cascades)), 0.0).rgb;
		}
		if (roughness > 0.2) {
			specular = mix(specular, textureLod(sampler2DArray(lightprobe_texture, linear_sampler), local_specular_posf, 0.0).rgb, (roughness - 0.2) * 1.25);
		}
