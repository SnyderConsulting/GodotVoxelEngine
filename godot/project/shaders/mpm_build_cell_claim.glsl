...
// Build a per-cell particle claim field from particle positions.
// Each occupied BCC cell stores (particle_index + 1). 0 means empty.
// This is used by mpm_g2p_advect.glsl to keep particles discrete (1 voxel per particle).
...
layout(set = 0, binding = 5, std430) buffer CellClaimOut {
    uint data[];
} cell_claim;
...
void main() {
    ...
    // If we ever see collisions here, it means the simulation already has overlapping particles.
    // Keep the first claimant; the g2p projection step will resolve to unique cells.
    atomicCompSwap(cell_claim.data[ai], 0u, p + 1u);
}
