...
// Build a coarse "sand occupancy" field from particle positions so water can collide
// against sand as if it were a voxel-solid at the current lattice resolution.
...
// 1u if the cell is "occupied by sand" this substep, else 0u.
layout(set = 0, binding = 5, std430) buffer SandOccOut {
    uint data[];
} sand_occ;

const uint SAND_MATERIAL = 1u;
// Conservative dilation: 14-neighborhood around the nearest BCC point.
// With a small collision margin, this makes sand effectively impermeable to water
// at the current lattice resolution.
...
// Fill downward a few layers so sand acts effectively impermeable (non-porous)
// at this resolution. This is a hacky "packing" approximation, not a real two-phase model.
const int DOWN_FILL_STEPS = 4; // each step is 2 cells in -Y (BCC lattice)
...
void main() {
    ...
    for (int i = 0; i < 15; i++) {
        ivec3 c0 = cell + OFFSETS[i];
        for (int k = 0; k < DOWN_FILL_STEPS; k++) {
            ivec3 c = c0 + ivec3(0, -2 * k, 0);
